...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        function Tab:CreateFloatingToggle(text, default, callback)
            ...
            local function createFloatingWindow()
                ...
                --// GLOW EXTERIOR INVISIBLE (Solo de placeholder)
                FloatingGlow = mk("Frame", {
                    Parent = Window.ScreenGui,
                    Size = UDim2.fromOffset(200 + 8, 40 + 8),
                    Position = UDim2.new(0.5, -108, 0.5, -28),
                    BackgroundColor3 = Theme.Accent,
                ...
                --// VENTANA PRINCIPAL (Cápsula más pequeña y transparente)
                FloatingWindow = mk("Frame", {
                    Parent = Window.ScreenGui,
                    Size = UDim2.fromOffset(200, 40),  -- 30% MÁS PEQUEÑA
                    Position = UDim2.new(0.5, -100, 0.5, -20),
                    BackgroundColor3 = Theme.Secondary,  -- ADAPTABLE AL TEMA
                    BackgroundTransparency = 0.35,  -- MÁS TRANSPARENTE
                    BorderSizePixel = 0,
                    ZIndex = 150
                })
                FloatingWindow:SetAttribute("ThemeRole", "Secondary")
                corner(FloatingWindow, 999)  -- Cápsula completa
                ...
                --// TEXTO (Centrado verticalmente, GothamBold, VISIBLE)
                local FloatLabel = mk("TextLabel", {
                    Parent = FloatingWindow,
                    Size = UDim2.new(0, 60, 1, 0),  -- ANCHO FIJO para que se vea completo
                    Position = UDim2.new(0, 8, 0, 0),
                    BackgroundTransparency = 1,
                    Text = text,
                    TextColor3 = Theme.Text,  -- Usa color del tema
                    Font = Enum.Font.GothamMedium,
                    TextSize = 11,  -- MÁS PEQUEÑO
                    TextXAlignment = Enum.TextXAlignment.Left,
                ...
                --// BOTÓN + (Fijar) - MÁS PEQUEÑO
                local PlusBtn = mk("TextButton", {
                    Parent = FloatingWindow,
                    Size = UDim2.fromOffset(16, 16),  -- REDUCIDO 40%
                    BackgroundColor3 = Theme.Accent,
                    Text = "+",
                    TextColor3 = Color3.fromRGB(255, 255, 255),
                    Font = Enum.Font.GothamBold,
                ...
                --// SWITCH (Track 40x20, Knob 16x16) - MÁS PEQUEÑO
                local FloatSwitch = mk("Frame", {
                    Parent = FloatingWindow,
                    Size = UDim2.fromOffset(40, 20),  -- REDUCIDO 30%
                    BackgroundColor3 = state and Theme.ToggleOn or Theme.AccentOff,
                    ZIndex = 151,
                    LayoutOrder = 4
                })
                ...
                local FloatKnob = mk("Frame", {
                    Parent = FloatSwitch,
                    Size = UDim2.fromOffset(16, 16),  -- REDUCIDO 30%
                    Position = state and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8),
                    BackgroundColor3 = Color3.new(1, 1, 1),
                    ZIndex = 152
                })
    ...
    function Window:SetTheme(themeName)
        ...
        --// Actualizar imagen de fondo - SIN ERROR
        if Window.ScreenGui and Window.ScreenGui.Parent then
            local contentArea = Window.ScreenGui:FindFirstChild("ContentArea")
            if contentArea then
                local bgArt = contentArea:FindFirstChild("BackgroundArt")
                if bgArt then
                    bgArt.Image = ThemeBackgroundImages[themeName] or ""
                    print("✅ Imagen de fondo actualizada al tema: " .. themeName)
                end
            end
        end
        ...
        --// 🐱 SISTEMA DE EFECTO DINÁMICO POR TEMA (MANTIENE EFECTOS ACTIVOS)
        if themeName == "CatV1" then
            self:SetTextEffect("CatRainbow")
            print("🐱 Tema Cat V1 activado + Efecto Rainbow activado")
        elseif themeName == "ErisV1" then
            self:SetTextEffect("ErisRainbow")
            print("🔴 Tema Eris V1 activado + Efecto Rainbow activado")
        elseif themeName == "PinkV1" or themeName == "PinkV2" or themeName == "PinkV3" then
            self:SetTextEffect("Rainbow")
            print("💕 Tema Pink activado + Efecto Rainbow activado")
        else
            --// Mantener el efecto actual si existe
            if CurrentTextEffect and CurrentTextEffect ~= "Off" then
                print("✨ Efecto " .. CurrentTextEffect .. " mantenido en tema " .. themeName)
            else
