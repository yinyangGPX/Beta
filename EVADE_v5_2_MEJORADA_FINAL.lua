--[[
    EVADE v5.2 MEJORADA - CONECTADA A YIN YANG v27 FINAL
...
local LoadSuccess = pcall(function()
    ...
    if not success then
        print("⚠️ GitHub no disponible, cargando localmente...")
...
-- ENHANCED JUMP
TabMovimiento:CreateToggle("⬆️ Enhanced Jump", Config.EnhancedJumpEnabled, function(State)
...
TabMovimiento:CreateLabel("💡 Salta automáticamente al correr", 10)
...
-- LAG SWITCH
TabMovimiento:CreateToggle("⚠️ Lag Switch [E]", Config.LagSwitchEnabled, function(State)
...
TabCreditos:CreateLabel("✨ INFORMACIÓN", 14)
...
TabCreditos:CreateLabel("©️ Todos los derechos reservados 2024", 10)
...
TabCreditos:CreateLabel("⚙️ INFORMACIÓN TÉCNICA", 12)
...
local function HandleFrontJump()
    ...
    if humanoid:GetState() == Enum.HumanoidStateType.Running then
        pcall(function()
            humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
            -- Front Jump: velocidad hacia adelante (ARREGLADO: controlable con WASD)
            local jumpForce = 50
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.E and Config.LagSwitchEnabled then
        print("⚠️ Lag Switch ACTIVADO (0.5s)")
        HandleLagSwitch()
    end
end)
...
local MainLoop = RunService.Heartbeat:Connect(function()
    if LagSwitchActive then return end -- Pausar si está en lag
